...
class Gallery {
    ...
    public function __construct( \Basepack\Core\Autoloader $loader, Array $params = null ) {
	$this->gallery_types();
	if ( count( $this->gallery_types ) > 1 ) {
	    add_action( 'wp_enqueue_media', array( $this, 'wp_enqueue_media' ) );
	    add_action( 'print_media_templates', array( $this, 'print_media_templates' ) );
	    add_shortcode( 'gallery', array( $this, 'setup_gallery' ) );
	}
    ...
    public function setup_gallery(Array $params ){
		...
		$params['type'] = $this->default_gallery;
            ...
            $params = apply_filters( 'gallery_params', $params );
	...
	if ( isset( $plugins ) && count( $plugins ) > 0 ) {
		...
		wp_register_script( 'jquery.' . strtolower( $plugin ), BASEPACK_PLUGIN_URL . 'modules/gallery/scripts/' . strtolower( $this->params['type'] ) . '/' . strtolower( $plugin ), 'jquery', false, true );
	...
	if ( isset( $styles ) && count( $styles ) > 0 ) {
		...
		wp_register_style( $style, BASEPACK_PLUGIN_URL . 'modules/gallery/scripts/' . strtolower( $this->params['type'] ) . '/' . strtolower( $style ) );
	...
	if ( ! wp_script_is( 'pwp-gallery-settings', 'registered' ) )
	    wp_register_script( 'pwp-gallery-settings', BASEPACK_PLUGIN_URL .'modules/gallery/gallery-settings.js', array( 'media-views' ), uniqid() );
